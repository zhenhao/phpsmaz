...
extern zend_module_entry smaz_module_entry;
#define phpext_smaz_ptr &smaz_module_entry

#ifdef PHP_WIN32
#	define PHP_SMAZ_API __declspec(dllexport)
#elif defined(__GNUC__) && __GNUC__ >= 4
#	define PHP_SMAZ_API __attribute__ ((visibility("default")))
#else
#	define PHP_SMAZ_API
#endif
...
ZEND_BEGIN_MODULE_GLOBALS(smaz)
	long  global_value;
	char *global_string;
ZEND_END_MODULE_GLOBALS(smaz)
*/

/* In every utility function you add that needs to use variables 
   in php_smaz_globals, call TSRMLS_FETCH(); after declaring other 
   variables used by that function, or better yet, pass in TSRMLS_CC
   after the last function argument and declare your utility function
   with TSRMLS_DC after the last declared argument.  Always refer to
   the globals in your function as SMAZ_G(variable).  You are 
   encouraged to rename these macros something shorter, see
   examples in any other php module directory.
*/

#ifdef ZTS
#define SMAZ_G(v) TSRMG(smaz_globals_id, zend_smaz_globals *, v)
#else
#define SMAZ_G(v) (smaz_globals.v)
#endif
